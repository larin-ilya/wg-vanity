...
# fx — слой цифрового шума
var _fx: ColorRect
var _fx_mat: ShaderMaterial
var _fx_glitch_t = 0.0
...
func _process(_delta: float) -> void:
	...
	if _fx_mat != null:
		if _fx_glitch_t > 0.0:
			_fx_glitch_t = max(0.0, _fx_glitch_t - _delta * 3.0)
			_fx_mat.set_shader_param("glitch", _fx_glitch_t)
...
func _build_mid() -> Control:
	...
	server_section = _server_section()
	pc.add_child(server_section)
...
func _fill_idle_hints() -> void:
	if idle_hints == null:
		return
	for c in idle_hints.get_children():
		idle_hints.remove_child(c)
		c.queue_free()
	var tips = [
	...
	if kind == "onion":
		tips = [
			"•  onion v3 = 56 символов a-z и 2-7, адрес выглядит как xxxx…d.onion",
			"•  «Обычный» пробует похожие символы: e→3, g→6, s→5, t→7, z→2…",
			"•  Найдя адрес, программа кладёт ключи в папку <адрес>.onion/ — Tor подхватит их как HiddenServiceDir",
		]
	for t in tips:
...
func _tile(grid: GridContainer, caption: String, key: String) -> void:
	...
	pad.add_child(vb)
	var cap_lbl = Style.label(caption, 10, Style.FAINT, true)
	vb.add_child(cap_lbl)
	var val = Style.label("—", 19, Style.TXT, true)
	vb.add_child(val)
	p.set_meta("key", key)
	p.set_meta("value", val)
	p.set_meta("caption", caption)
	p.set_meta("caption_lbl", cap_lbl)
	grid.add_child(p)

func _set_tile_caption(key: String, text: String) -> void:
	for child in stat_grid.get_children():
		if child.get_meta("key") == key:
			(child.get_meta("caption_lbl") as Label).text = text
			child.set_meta("caption", text)
			return
...
func _build_overlay() -> void:
	...
	var ov_title = Style.label("НАЙДЕН КРАСИВЫЙ КЛЮЧ", 24, Style.TXT, false, true)
	head.add_child(ov_title)
	overlay.set_meta("ov_title", ov_title)
	vb.add_child(head)
...
func _keycard(caption: String, meta_name: String) -> PanelContainer:
	...
	panel.add_child(vb)
	var cap = Style.label(caption, 11, Style.FAINT, true)
	vb.add_child(cap)
	overlay.set_meta(meta_name + "_cap", cap)
...
func _build_fx() -> void:
	# слой цифрового шума поверх всего (последний ребёнок = рисуется сверху)
	_fx = ColorRect.new()
	_fx.set_anchors_preset(Control.PRESET_WIDE)
	_fx.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_fx.color = Color(1, 1, 1, 1)
	var shader = load("res://shaders/noise.gdshader")
	if shader != null:
		_fx_mat = ShaderMaterial.new()
		_fx_mat.shader = shader
		_fx.material = _fx_mat
		_fx_mat.set_shader_param("intensity", 0.05)
		_fx_mat.set_shader_param("scan_strength", 0.5)
		_fx_mat.set_shader_param("glitch", 0.0)
	add_child(_fx)

func _fx_glitch() -> void:
	_fx_glitch_t = 1.0
	if _fx_mat != null:
		_fx_mat.set_shader_param("glitch", 1.0)
...
func _on_kind_changed() -> void:
	kind = "wg" if seg_type.get_selected() == 0 else "onion"
	_rebuild_chips()
	if kind == "onion":
		word_edit.placeholder_text = "например: cook, tor, coin… (только a-z и 2-7)"
		word_edit.max_length = 55
		seg_mode.set_options(["Обычный — с заменами", "Строгий — точный префикс"])
	else:
		word_edit.placeholder_text = "например: lenovo, cook, dark…"
		word_edit.max_length = 34
		seg_mode.set_options(["Обычный — с заменами", "Строгий — точное слово"])
	if server_section != null:
		server_section.visible = kind == "wg"
	_set_tile_caption("checked", "ПРОВЕРЕНО АДРЕСОВ" if kind == "onion" else "ПРОВЕРЕНО КЛЮЧЕЙ")
	_fill_idle_hints()
	_update_word_hint()
	_save_settings()
...
func _rebuild_chips() -> void:
	...
	for c in chips_box.get_children():
		chips_box.remove_child(c)
		c.queue_free()
	var words = ["cook", "test", "vpn", "lenovo", "coin", "dark"] \
		if kind == "wg" else ["cook", "tor", "vpn", "onion", "coin", "dark"]
	for w in words:
		var b = Style.button(w.to_upper() if kind == "wg" else w, 11, true)
		b.rect_min_size = Vector2(0, 26)
...
func _update_word_hint() -> void:
	...
	var alphabet = 32.0 if kind == "onion" else 64.0
	var variants = _count_variants(w, strict)
	var L = w.length()
	var exp_keys = pow(alphabet, L) / max(1.0, float(variants))
	var noun = "адресов" if kind == "onion" else "ключей"
	var t = "вариантов префиксов: "
	t += _fmt_num(variants)
	t += "  ·  " + ("строгий" if strict else "обычный")
	t += "  ·  в среднем ~%s %s" % [_fmt_num(exp_keys), noun]
	word_hint.text = t
...
func _count_variants(w: String, strict: bool) -> float:
	...
	for ch in w:
		var n = 1
		if not strict:
			var subs = _SUBS_ONION if kind == "onion" else _SUBS
			if subs.has(ch):
				n = subs[ch].size()
		product *= float(n)
	return product
...
func _validate() -> Dictionary:
	...
	if kind == "onion":
		w = w.to_lower()
	if w == "":
		return {"ok": false, "err": "Введите слово/префикс для поиска"}
	if kind == "onion":
		var oni = "abcdefghijklmnopqrstuvwxyz234567"
		for ch in w:
			if oni.find(ch) == -1:
				return {"ok": false, "err": "Для .onion нельзя «%s» — только a-z и 2-7" % ch}
	else:
		var allowed = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
		for ch in w:
			if allowed.find(ch) == -1:
				return {"ok": false, "err": "Недопустимый символ «%s» — только латиница и цифры" % ch}
	if save_toggle.is_on() and kind == "wg":
...
func _start_search() -> void:
	...
	state_sub.text = "проверяем " + ("адреса" if kind == "onion" else "ключи") + " на %s ядрах — обычно секунды или минуты" % workers_lbl.text
	state_sub.add_color_override("font_color", Style.CYAN)
	_shown_reset()
	_log("▶ Старт " + ("поиска .onion" if kind == "onion" else "поиска") + ": «%s» · режим: %s · ядер: %s" % [
...
func _on_found(m: Dictionary) -> void:
	...
	ring.set_mode(2)
	var is_onion = m.get("kind", "wg") == "onion"
	var prefix: String = str(m.get("prefix", ""))
	if is_onion:
		state_title.text = "НАЙДЕН .ONION!"
		state_sub.text = "адрес с префиксом " + prefix
	else:
		state_title.text = "НАЙДЕНО!"
		state_sub.text = "ключ с префиксом " + prefix
	state_sub.add_color_override("font_color", Style.MINT)
	_set_tile_text("time", Style.fmt_dur(float(m.get("elapsed", 0))))
	_set_tile_text("checked", Style.fmt_int(m.get("checked", 0)))
	var found_word = "адрес" if is_onion else "ключ"
	_log("🎉 Найден %s с префиксом «%s»! Проверено: %s · время: %s" % [
		found_word, prefix, Style.fmt_int(m.get("checked", 0)),
		Style.fmt_dur(float(m.get("elapsed", 0)))], Style.MINT)

	# заголовок карточки результата
	var title_lbl: Label = overlay.get_meta("ov_title")
	title_lbl.text = "НАЙДЕН КРАСИВЫЙ .ONION" if is_onion else "НАЙДЕН КРАСИВЫЙ КЛЮЧ"

	# правая колонка (QR) видна только для WireGuard
	var rcol: VBoxContainer = overlay.get_meta("ov_rcol")
	rcol.visible = not is_onion
	...
	if files.size() == 0:
		if is_onion:
			lines.append("сохранение выключено — скопируйте адрес и seed кнопками ниже")
		else:
			lines.append("сохранение выключено — скопируйте ключи кнопками ниже")
	ov_files.text = "\n".join(lines)

	if is_onion:
		var onion_s = str(m.get("onion", "")) + ".onion"
		var seed_s = str(m.get("seed_b64", ""))
		ov_prefix.text = prefix + "…  —  адрес начинается с этого префикса"
		(_get_meta_label("ov_pub") as Label).text = onion_s
		(_get_meta_label("ov_priv") as Label).text = seed_s
		(_get_meta_label("ov_pub_cap") as Label).text = "onion-адрес (56 символов)"
		(_get_meta_label("ov_priv_cap") as Label).text = "seed ed25519 (base64)"
		overlay.set_meta("last_pub", onion_s)
		overlay.set_meta("last_priv", seed_s)
		var btn1: Button = overlay.get_meta("ov_btn1")
		var btn2: Button = overlay.get_meta("ov_btn2")
		btn1.text = "Копировать адрес"
		btn2.text = "Копировать seed"
		_set_qr("")
	else:
		var pub: String = m.get("public_key", "")
		var priv: String = m.get("private_key", "")
		ov_prefix.text = "wg:" + prefix + "   —   публичный ключ начинается с этого префикса"
		(_get_meta_label("ov_pub") as Label).text = pub
		(_get_meta_label("ov_priv") as Label).text = priv
		(_get_meta_label("ov_pub_cap") as Label).text = "Публичный ключ клиента"
		(_get_meta_label("ov_priv_cap") as Label).text = "Приватный ключ клиента"
		overlay.set_meta("last_pub", pub)
		overlay.set_meta("last_priv", priv)
		var btn1: Button = overlay.get_meta("ov_btn1")
		var btn2: Button = overlay.get_meta("ov_btn2")
		btn1.text = "Копировать приватный"
		btn2.text = "Копировать публичный"
		_set_qr(m.get("qr_png_b64", ""))
...
func _apply_settings() -> void:
	...
	seg_type.set_selected(int(cfg.get_value("search", "kind", 0)))
	_on_kind_changed()
...
func _save_settings() -> void:
	...
	cfg.set_value("search", "kind", seg_type.get_selected())
...
func _demo_found() -> void:
	...
	if w == "":
		w = "demo"
	if kind == "onion":
		var onion = _fake_onion(w)
		_on_found({
			"kind": "onion",
			"prefix": w.to_lower(),
			"onion": onion,
			"seed_b64": _fake_b64(),
			"public_key_b64": _fake_b64(),
			"checked": _sim_checked,
			"elapsed": _sim_elapsed,
			"worker_id": 1,
			"files": [
				onion + ".onion/hostname",
				onion + ".onion/hs_ed25519_public_key",
				onion + ".onion/hs_ed25519_secret_key",
				"onion_keys_log.txt",
			],
		})
		return
...
func _fake_onion(prefix: String) -> String:
	var chars = "abcdefghijklmnopqrstuvwxyz234567"
	var s = prefix.to_lower()
	while s.length() < 56:
		s += chars[randi() % chars.length()]
	return s.substr(0, 56)
